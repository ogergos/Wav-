...
export default {
  ...
  computed: {
    sortItem() {
      this.$store.state.stclothes.sort((a, b) => a.numberprice - b.numberprice);
    },
    sortItem1() {
      this.$store.state.stclothes.sort((a, b) => b.numberprice - a.numberprice);
    },
    sortItem2() {
      this.$store.state.stclothes.sort(() => 0.5 - Math.random());
    },
  },
};
</script>
  ...
  <main>
    ...
    <div class="content">
      <Item1
        ...
        :gender="this.$store.state.stclothes[index].gender"
        :type="this.$store.state.stclothes[index].type"
        :price="this.$store.state.stclothes[index].price"
        v-for="(item, index) in this.$store.state.stclothes"
        :key="index"
        @add-to-shop="this.$store.commit('shortsleeveincrement', index)"
        class="imgList"
      />
    </div>
  </main>
