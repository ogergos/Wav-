...
export default {
  ...
  computed: {
    sortItem() {
      this.$store.state.pant.sort((a, b) => a.numberprice - b.numberprice);
    },
    sortItem1() {
      this.$store.state.pant.sort((a, b) => b.numberprice - a.numberprice);
    },
    sortItem2() {
      this.$store.state.pant.sort(() => 0.5 - Math.random());
    },
  },
};
</script>
  ...
  <main>
    ...
    <div class="content">
      <Item1
        ...
        :gender="this.$store.state.pant[index].gender"
        :type="this.$store.state.pant[index].type"
        :price="this.$store.state.pant[index].price"
        v-for="(item, index) in this.$store.state.pant"
        :key="index"
        @add-to-shop="this.$store.commit('pantsincrement', index)"
        class="imgList"
      />
    </div>
  </main>
