...
export default {
  ...
  computed: {
    sortItem() {
      this.$store.state.longclothes.sort((a, b) => a.numberprice - b.numberprice);
    },
    sortItem1() {
      this.$store.state.longclothes.sort((a, b) => b.numberprice - a.numberprice);
    },
    sortItem2() {
      this.$store.state.longclothes.sort(() => 0.5 - Math.random());
    },
  },
};
</script>
  ...
  <main>
    ...
    <div class="content">
      <Item1
        ...
        :gender="this.$store.state.longclothes[index].gender"
        :type="this.$store.state.longclothes[index].type"
        :price="this.$store.state.longclothes[index].price"
        v-for="(item, index) in this.$store.state.longclothes"
        :key="index"
        @add-to-shop="this.$store.commit('longsleeveincrement', index)"
        class="imgList"
      />
    </div>
  </main>
