...
export default {
  ...
  computed: {
    sortItem() {
      this.$store.state.girlDress1.sort((a, b) => a.numberprice - b.numberprice);
    },
    sortItem1() {
      this.$store.state.girlDress1.sort((a, b) => b.numberprice - a.numberprice);
    },
    sortItem2() {
      this.$store.state.girlDress1.sort(() => 0.5 - Math.random());
    },
  },
};
</script>
  ...
  <main>
    ...
    <div class="content">
      <Item1
        ...
        :gender="this.$store.state.girlDress1[index].gender"
        :type="this.$store.state.girlDress1[index].type"
        :price="this.$store.state.girlDress1[index].price"
        v-for="(item, index) in this.$store.state.girlDress1"
        :key="index"
        @add-to-shop="this.$store.commit('girlDressincrement', index)"
        class="imgList"
      />
    </div>
  </main>
